...
class MRKMeansUpdateCentroids(MRJob):
    ...
    def get_centroids(self):
        ...
        #print "Centroids is now:" , centroids
        f.close()
        return centroids

    def assign_cluster(self, _, line):
        l = line.split('~;')
        #print "Inside assign_cluster:"
        #print "Line:", l
        if len(l) == 1:
            return
        point = numpy.array([float(x) for x in l[3:]])
        centroids = self.get_centroids()
        #This one uses the euclidean distance
        distances = [numpy.linalg.norm(point - c) for c in centroids]
        cluster = numpy.argmin(distances)
        print "Cluster Assignment:", cluster, "Point:", point.tolist()
